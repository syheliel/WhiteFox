...
from src.db.factory import BaseEmbedding, ChromaVectorDB
import tqdm
...
def get_api_docs(apis:List[str],embedding:BaseEmbedding, vector_db:ChromaVectorDB) -> List[str]:
    ...
    for api in apis:
        ...
        docs = vector_db.query_by_emb(query_embedding, n_results=2)["documents"][0]
        results.extend(docs)
    return results
...
def main(input_dir: str, output_dir: str, model: str, device: str, gen_num: int, temperature: float, use_vllm: bool):
    ...
    for spec_p in tqdm.tqdm(all_specs):
        ...
        for idx, choice in enumerate(prompt.choices):
            ...
            with open(func_dir / (out_name + ".doc"), "w") as f:
                f.write("\n".join([f"```doc\n{api_doc}\n```" for api_doc in api_docs])) # type: ignore
