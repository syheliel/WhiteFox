...
for optim_name in 'inductor' 'group-batch'; do
    python prompt_gen.py --optpath=optim/inductor-${optim_name}.json --nlpath=${nl_path} --mode=${mode}2test --template=${template_dir}/starcoder-${mode}2test-oneshot-pattern.txt --outdir=${generate_dir}/torch-inductor
done
