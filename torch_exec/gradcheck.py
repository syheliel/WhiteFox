"""
The original version of this file is torch/autograd/gradcheck.py
"""
...
def _iter_tensors(
    ...
    only_requiring_grad: bool = False,
...
def _get_numerical_jacobian(
    ...
    Args:
        ...
        kind: 0 means central derivative, 1 means left, and 2 means right (extended)
...
def get_numerical_jacobian(fn, inputs, eps=1e-4):
    ...
    if has_any_complex_output:
        real_fn, imag_fn = _real_and_imag_output(fn)
        real_jacobian = _get_numerical_jacobian(real_fn, inputs, eps=eps)
        imag_jacobian = _get_numerical_jacobian(imag_fn, inputs, eps=eps)
        return real_jacobian + imag_jacobian
    else:
        return _get_numerical_jacobian(fn, inputs, eps=eps)


def _compute_numerical_gradient(fn, entry, v, norm_v, nbhd_checks_fn, outputs, kind=0):
    ...
    return tuple(compute(a, b, kind) for (a, b) in zip(outa, outb))
...
def _prepare_input(
    input: torch.Tensor,
    maybe_perturbed_input: Optional[torch.Tensor],
    fast_mode=False,
) -> torch.Tensor:
...
def get_numerical_jacobian_wrt_specific_input(
    ...
    for x, idx, d_idx in _iter_tensor(input):
        ...
        nbhd_checks_fn = functools.partial(
            check_outputs_same_dtype_and_shape, idx=idx, eps=eps
        )
        jvp_fn = _get_numerical_jvp_fn(
            wrapped_fn, input_to_perturb, eps, nbhd_checks_fn, outputs, kind
        )
        ...
        if all_u:
            ...
            for i, fw_grad in enumerate(fw_grads):
                for lin_idx, grad_idx in enumerate(
                    ...
                    # print(lin_idx,grad_idx,fw_grad)
                    # print(dual_inputs[0])
                    raw_outputs = _as_tuple(fn(*dual_inputs))
                    dual_outputs = filter(_is_float_or_complex_tensor, raw_outputs)
                    for index_o, d_o in enumerate(dual_outputs):
...
def _get_numerical_jvp_fn(
    wrapped_fn, input_to_perturb, eps, nbhd_checks_fn, outputs, kind=0
):
...
def _get_numerical_jvp_wrt_specific_input(
    fn, input_idx, inputs, u, eps, outputs, is_forward_ad=False, kind=0
) -> List[torch.Tensor]:
    ...
    jvp_fn = _get_numerical_jvp_fn(
        wrapped_fn, input_to_perturb, eps, nbhd_checks_fn, kind, outputs
    )
...
def _check_outputs(outputs) -> None:
    if any(
        t.layout == torch.sparse_coo for t in outputs if isinstance(t, torch.Tensor)
    ):
                ...
                for dual_output in dual_outputs:
                    ...
                    if tangent_out is not None:
                        ...
                        ret.append(
                            torch.zeros(
                                [],
                                dtype=primal_out.dtype,
                                device=primal_out.device,
    ...
    *,
    ...
    numerical = _transpose(
        _get_numerical_jacobian(
            func, tupled_inputs, outputs, eps=eps, is_forward_ad=use_forward_ad
        )
    )
...
def _adjusted_atol(atol, u, v):
    ...
    sum_v = (
        1.0
        if v is None
        else torch.sparse.sum(v) if v.layout == torch.sparse_coo else v.sum()
    )
...
def get_jacobians(func, inputs, backward_ad=True):
    """
    Return the jacobians of func with inputs
    Args:
        @param func: the function
        @param inputs
        @param backward_ad: if True, compute the jacobians in backward AD;
                            otherwise, return jacobians in forward AD
    """
    tupled_inputs = _as_tuple(inputs)
    ...
    # FIXME: not a perfect version
    complex_inp_indices = [
        i
        for i, inp in enumerate(tupled_inputs)
        if is_tensor_like(inp) and inp.is_complex()
    ]
    if has_any_complex_output or complex_inp_indices:
        real_fn, imag_fn = _real_and_imag_output(func)
        ...
        imag_outputs = _differentiable_outputs(imag_func_out)
        jacobians.extend(
            _get_jacobian_helper(
                func,
                imag_func_out,
                imag_inputs,
                imag_outputs,
                backward_ad=backward_ad,
            )
        )
        ...
        jacobians.extend(
            _get_jacobian_helper(
                func, func_out, tupled_inputs, outputs, backward_ad=backward_ad
            )
        )
    ...
    #         imag_func_out = imag_fn(*tupled_inputs)
    #         imag_outputs = _differentiable_outputs(imag_func_out)
    #         jacobians.extend(_get_jacobian_helper(func, imag_func_out, tupled_inputs, imag_outputs, backward_ad=backward_ad))

    #         real_func_out = real_fn(*tupled_inputs)
    #         real_outputs = _differentiable_outputs(real_func_out)
    #         jacobians.extend(_get_jacobian_helper(func, real_func_out, tupled_inputs, real_outputs, backward_ad=backward_ad))
    #     else:
    #         jacobians.extend(_get_jacobian_helper(func, func_out, tupled_inputs, outputs, backward_ad=backward_ad))
    # else:
    ...
    #         imag_func_out = imag_fn(*imag_inputs)
    #         diff_imag_func_out = _differentiable_outputs(imag_func_out)
    #         jacobians.extend(_get_jacobian_helper(func, imag_func_out, imag_inputs, diff_imag_func_out, backward_ad=backward_ad))

    #         real_inputs = [inp.real if is_tensor_like(inp) and inp.is_complex() else inp for inp in tupled_inputs]
    #         real_func_out = real_fn(*real_inputs)
    #         diff_real_func_out = _differentiable_outputs(real_func_out)
    #         jacobians.extend(_get_jacobian_helper(func, real_func_out, real_inputs, diff_real_func_out, backward_ad=backward_ad))
    #     else:
    #         jacobians.extend(_get_jacobian_helper(func, func_out, tupled_inputs, outputs, backward_ad=backward_ad))
    return jacobians


def _get_jacobian_helper(func, func_out, tupled_inputs, outputs, backward_ad=True):
    def get_tensors(input):
        if is_tensor_like(input):
            return [input]
        elif isinstance(input, (tuple, list)):
            tensor_list = []
            for i in input:
                tensor_list.extend(get_tensors(i))
            return tensor_list
        else:
            return []
    ...
    if backward_ad:
        # FIXME: the outputs is strange
        temp = []
        for output in outputs:
            temp.extend(get_tensors(get_analytical_jacobian(tupled_inputs, output)))
        return temp
    else:
        return get_tensors(
            _get_analytical_jacobian_forward_ad(func, tupled_inputs, func_out)
        )
