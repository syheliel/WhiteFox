...
if __name__ == "__main__":
    ...
    args.add_argument(
        "--data_dir", type=str, default="_results-new"
    )  # FIXME: change this
    ...
    if base_file.exists():
        ...
        for optim in output_keys:
            if f"{optim}_selected" not in base_trigger_codes:
                continue
    ...
    # Merge the new trigger info with the old one.
    for optim in output_keys:
        if optim not in trigger_codes and optim not in base_trigger_codes:
            continue
        if optim not in trigger_codes:
            ...
            merged_trigger_codes[optim] = (
                base_trigger_codes[optim] + trigger_codes[optim]
            )
